...
class App extends React.Component {
  ...
  render ()
  {
    return (
      <div className="App">
        ...
        <button onClick={()=> this.update(false,0,1)}>Strike</button>
  ...
  {
    if(_end) return this.ball = this.strike = this.out = 0;
    if(_reset) this.ball = this.strike = 0;
    this.strike += _strike + (this.strike < 2 ? _foul : 0);
    this.ball += _ball
    this.out += _out;
    if(this.strke > 2) return this.update(true,1) //reset at bat and add an out
